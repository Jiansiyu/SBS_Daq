...
#define MAX_SDATA 1024
			...
			for (kk = clp_clock0; kk < clp_clock1; kk += clp_clockd) { // loop on clock phases
				...
				for (j = 0; j < 16; j++) { // 16 = number of ADC channel in one MPD
					if (mpdGetApvEnableMask(i) & (1 << j)) { // apv is enabled
						mpdFIFO_Samples(i, j, sdata, &scount, MAX_SDATA,
								&error_count);
						...
						fprintf(fout,
								"# MPD_ADC_COUNT_CLOCK_TOUT: %d %d %d %d %d\n",
								i, j, scount, kk, rtout);
	...
	if (acq_mode & 0x8) {
		...
		for (k = 0; k < fnMPD; k++) { // only active mpd set
			...
			// set ADC gain
			mpdADS5281_SetGain(i, 0, h_gain, h_gain, h_gain, h_gain, h_gain,
					h_gain, h_gain, h_gain);
			mpdADS5281_SetGain(i, 1, h_gain, h_gain, h_gain, h_gain, h_gain,
					h_gain, h_gain, h_gain);
			...
			for (j = 0; j < 16; j++) {
				...
				if (error_count != OK) {
					printf(
							"ERROR: reading histogram data from MPD %d ADCch %d\n",
							i, j);
					continue;
				}

				printf(
						" ### histo peaks integral MPD/ADCch = %d / %d (total Integral= %d)\n",
						i, j, hcount);
				sch0 = -1;
				sch1 = 0;
	...
	if (acq_mode & 0x4) {
		...
		fprintf(fout,
				"# MODE=%d\n# OUTPUT=mpd clockphase adc gain ch0 ... ch4095\n",
				acq_mode);

		for ( clockphase = 0; clockphase <= 64; clockphase +=1) { // loop on clock phases
			...
			for (k = 0; k < fnMPD; k++) { // only active mpd set
				...
				// load pedestal and thr default values
				mpdPEDTHR_Write(i);
				mpdDELAY25_Set(i, clockphase, clockphase);
				//
				// set ADC gain
				mpdADS5281_SetGain(i, 0, h_gain, h_gain, h_gain, h_gain, h_gain,
						h_gain, h_gain, h_gain);
			...
			for (j = 0; j < 16; j++) {  // loop on adc channels
				...
				for (k = 0; k < fnMPD; k++) { // only active mpd set
					...
					printf(
							" ### histo peaks integral MPD/ADCch = %d / %d (total Integral= %d) clockphase =%d\n",
							i, j, hcount, clockphase);
					sch0 = -1;
					sch1 = 0;

					fprintf(fout, "# MPD=%d \n# clockphase = %d \n# ADC=%d\n# GAIN=%d\n", i, clockphase , j, h_gain);
					fprintf(fout, "%d %d %d %d", i, clockphase , j, h_gain);
...
#define FILE_VERSION 0x1
		...
		do { // simulate loop on trigger
			...
			for (kk = 0; kk < fnMPD; kk++) { // only active mpd set
				...
				} while ((rdone == 0) && (error_count == -1)
						&& (rtout < MPD_TIMEOUT)); // timeout can be changed
				//	mpdTRIG_Disable(i);
				...
				if (rdone) { // data need to be written on file
					...
					for (j = 0; j < mpdGetNumberAPV(i); j++) { // loop on APV (ADC channels)
						...
						for (h = 0; h < mpdApvGetBufferSample(i, j); h++) { // loop on samples
							...
							k++;
							e_head = ((e_head0 & 0xfff) << 4)
									| (mpdApvGetAdc(i, j) & 0xf);
							fprintf(fout, "%x\n", e_head0 | HEADER_TAG);
							// fwrite e_head
							for (m = 0; m < e_size - 2; m++) {
								e_data32[m] = 0x80000 | ((i << 12) & 0x7F000)
										| (mpdApvGetBufferElement(i, j, k)
												& 0xfff);
								fprintf(fout, "%d\n",
										(mpdApvGetBufferElement(i, j, k) & 0xfff)
												| DATA_TAG);
								k++;
							}
							// fwire e_data32 (e_size-2)
							e_trai = 0x100000 | ((i & 0x1F) << 12)
									| (mpdApvGetBufferElement(i, j, k) & 0xfff);
	...
	if ((acq_mode & 1) && (mpdGetEventBuilding(mpdSlot(0)) != 0)) {
		...
		fprintf(fout, "%x\n",
				mpdGetTriggerNumber(mpdSlot(0))
						* mpdApvGetPeakMode(mpdSlot(0)));
		...
		sig_ctrl = 1;
		printf(
				" ============ START ACQ MPD EVT_BUILDER (#mpd = %d)===========\n",
				fnMPD);
		...
		verbose_level = 2;
		do { // simulate loop on trigger
			usleep(10000); // wait for event
			// event occurred ... need some trigger logic
			rtout = 0;

			if ((evt % 100) == 0 && evt != 0)
				printf(
						" ---- At least %d events acquired on each active MPDs -----\n",
						evt);
...
//	blen = mpdApvGetBufferAvailable(i, 0);
				...
				if (verbose_level > 1)
					printf(
							"MPD %d: Buffer LEN = %d (bytes) -> %d (32b words)\n",
							i, blen, blen / 4);
				nwread = 0;
				...
				if (UseSdram) {
					...
					mpdSDRAM_GetParam(i, &sd_init, &sd_overrun, &sd_rdaddr,
							&sd_wraddr, &sd_nwords);
					if (verbose_level > 0)
						printf(
								"MPD %d: SDRAM status: init=%d, overrun=%d, rdaddr=0x%x, wraddr=0x%x, nwords=%d\n",
								i, sd_init, sd_overrun, sd_rdaddr, sd_wraddr,
								sd_nwords);
					...
					if (verbose_level > 0)
						printf(
								"MPD %d: OBUFF status: empty=%d, full=%d, nwords=%d : nblock %d\n",
					...
					if (obuf_nblock > 0) { // read data
						...
						if (verbose_level > 0)
							printf(
									"MPD %d: Data waiting to be read in obuf: %d (32b-words)\n",
									i, nwords);
...
//  	    mpdOBUF_GetFlags(i, &empty, &full, &nwords);	// DUMMY
						...
						if (verbose_level > 0)
							printf(
									"MPD %d: try to read %d 32b-words 128b-aligned from obuf, got %d 32b-words\n",
									i, nwords, nwread);

						if (nwords != nwread) {
							printf(
									"MPD %d: Error: 32bit-word read count does not match %d %d\n",
									i, nwords, nwread);
						}
					...
					if (!empty) { // read fifo
						nwread = blen / 4;
						mpdFIFO_ReadSingle(i, 0,
								mpdApvGetBufferPointer(i, 0, 0), &nwread, 20);
						if (nwread == 0) {
							printf(
									"MPD %d: Error: word read count is 0, while some words are expected back\n",
									i);
						}
					}
				...
				if (nwread > 0) { // data need to be written on file
					...
					if (verbose_level > 1)
						printf(
								"MPD Slot: %d (dump on screen first 24 and last 24 words)\n",
								i); // slot
					// fprintf(fout,"%x\n", i | MPD_TAG);
					...
					if (verbose_level > 0)
						printf(
								"MPD %d: nwords=%d nwcount=%d zero_count=%d evt %d\n",
...
int DMA_Init(int id, int fast_readout) {
	...
	printf(
			...
			fBufSize, (uint32_t) dmaHdl, (uint32_t) physMemBase,
			(uint32_t) fBuffer, (uint32_t) vmeAdrs);
