...
/* vmeBusLock/Unlock function pointers */
static LONG_FUNCTIONPTR remexVmeBusLock;
static LONG_FUNCTIONPTR remexVmeBusUnlock;
static int vmePresent=0;
  ...
  if((res != (-1)) && (res != 0))
    {
      ...
      remexVmeBusLock = (LONG_FUNCTIONPTR) res;

      res = (long)dlsym((void *)handler, "vmeBusUnlock");
      if((res != (-1)) && (res != 0))
	{
	  printf("%s: Found vmeBusUnlock()\n",__FUNCTION__);
	  remexVmeBusUnlock = (LONG_FUNCTIONPTR) res;
      ...
      else
	{
	  ...
	  remexVmeBusUnlock=NULL;
	  remexVmeBusLock=NULL;
	  vmePresent=0;
	}
    }
  else
    {
      printf("%s: ERROR: >%s()< routine not found\n",__FUNCTION__,"vmeBusLock");
      remexVmeBusUnlock=NULL;
      remexVmeBusLock=NULL;
      vmePresent=0;
    }
...
remexCallback(void *msg, void *arg)
{
  ...
  cMsgGetInt32(msg, "useVmeBusLock", (int *)&useVmeBusLock);

  if(strcmp(msubject,"ExecuteFunction")==0)
    {
      res = 
	remexExecuteFunction(command, useVmeBusLock, rettype, &myretVal, &errCode);
    ...
    {
      res = 
	remexGetVar(command, useVmeBusLock, rettype, &myretVal, &errCode);
    }
  else if(strcmp(msubject,"LoadLibrary")==0)
    {
      res = 
	remexLoadLibrary(command, useVmeBusLock, rettype, &myretVal, &errCode);
    }
 ...
 *    
 *          lockFlag - Whether (1) or not (0) to lock the vmeBus 
		     ...
		     enum remexErrCode *errCode)
{
  ...
  if( (lockFlag==1) && (vmePresent==1) )
    {
#ifdef DEBUG
      printf("DEBUG: vmeBusLock\n");
#endif
      remexVmeBusLock();
    }
