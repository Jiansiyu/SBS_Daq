...
sspPrintMigStatus(int id)
{
  ...
  /* soft reset (resets EB and fifo) */
  status = vmeRead32(&pSSP[id]->Mig.Status);
  printf("%s: Mig Status = 0x%08X\n", __func__, status); 
...
sspCheckAddresses(int id)
{
  ...
  offset = ((unsigned int) &pSSP[id]->EB) - base;
  expected = 0x2000;
  if(offset != expected)
    printf("%s: ERROR pSSP[%d]->EB not at offset = 0x%x (@ 0x%x)\n",
	   __FUNCTION__,id,expected,offset);

  offset = ((unsigned int) &pSSP[id]->MPD[0]) - base;
  expected = 0x3000;
  if(offset != expected)
    printf("%s: ERROR pSSP[%d]->MPD[0] not at offset = 0x%x (@ 0x%x)\n",
	   __FUNCTION__,id,expected,offset);

  offset = ((unsigned int) &pSSP[id]->MPDregs[0]) - base;
  expected = 0x40000;
  if(offset != expected)
    printf("%s: ERROR pSSP[%d]->MPDregs[0] not at offset = 0x%x (@ 0x%x)\n",
	   __FUNCTION__,id,expected,offset);

  offset = ((unsigned int) &pSSP[id]->MPDSelector) - base;
  expected = 0x7FFFC;
  if(offset != expected)
    printf("%s: ERROR pSSP[%d]->MPDSelector not at offset = 0x%x (@ 0x%x)\n",
	   __FUNCTION__,id,expected,offset);
...
sspGStatus(int rflag)
{
  ...
  SSP_regs st[MAX_VME_SLOTS+1];
  int channelUp[MAX_VME_SLOTS+1][SSP_SER_NUM];
  int powerUp[MAX_VME_SLOTS+1][SSP_SER_NUM];
  int rcvTrigData[MAX_VME_SLOTS+1][SSP_SER_NUM];
  ...
  if(showPortStatus)
    {
      ...
      printf("                Channel  Rcv Trig   Trigger        Lane bit errors\n");
      printf("Sl- P    ID     Status     Data     Latency      0     1     2     3\n");
      printf("--------------------------------------------------------------------------------\n");
      for(issp=0; issp<nSSP; issp++)
	{
	  id = sspSlot(issp);
...
int
sspMpdFiberLinkReset(int id, unsigned int mpdmask)
{
  int impd=0;
  if(id==0) id=sspID[0];
  if((id<=0) || (id>21) || (pSSP[id]==NULL)) 
...
int
sspMpdEnable(int id, unsigned int mpdmask)
{
  int impd=0;
  if(id==0) id=sspID[0];
  if((id<=0) || (id>21) || (pSSP[id]==NULL)) 
  ...
  for(impd=0; impd<32; impd++)
	...
	printf("BusyTh set!!!0x%08X\n", vmeRead32(&pSSP[id]->MPD[impd].EBBusyThreshold));
	}
    }
  SSPUNLOCK;
...
int
sspMpdDisable(int id, unsigned int mpdmask)
{
  int impd=0;
  if(id==0) id=sspID[0];
  if((id<=0) || (id>21) || (pSSP[id]==NULL)) 
  ...
  for(impd=0; impd<32; impd++)
    {
      if(mpdmask & (1<<impd))
	vmeWrite32(&pSSP[id]->MPD[impd].EBCtrl, 0);
    }
  SSPUNLOCK;
...
int
sspMpdReadRegs(int id, int impd)
{
...
static int sspMpdFiberSelected[MAX_VME_SLOTS+1];
static int sspMpdFiberSelected_initd=0;

unsigned int
sspMpdReadReg(int id, int impd, unsigned int reg)
{
  ...
  if(sspMpdFiberSelected_initd==0)
    {
      memset(&sspMpdFiberSelected,-1,sizeof(sspMpdFiberSelected));
      sspMpdFiberSelected_initd=1;
    }

  if(impd!=sspMpdFiberSelected[id])
    {
      vmeWrite32(&pSSP[id]->MPDSelector, impd);
      sspMpdFiberSelected[id]=impd;
    }
...
int
sspMpdWriteReg(int id, int impd, unsigned int reg, unsigned int value)
{
...
sspMpdPrintStatus(int id)
{
  ...
  for(impd=0; impd<32; impd++)
	...
	{
	  printf("%3d    ",(mr[impd].Status & MPD_STATUS_SOFTERRORS)>>12);
	}
      else
	printf("---    ");
...
int
sspGetMpdMaxRxLen(int id, int impd)
{
  int result;
...
int
sspGetEbStatus(int id, unsigned int *blockcnt, unsigned int *wordcnt, unsigned int *eventcnt)
{
