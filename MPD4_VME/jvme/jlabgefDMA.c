...
*/
int
jlabgefDmaConfig(unsigned int addrType, unsigned int dataType, unsigned int sstMode)
{
...
*/
int
jlabgefDmaSend(unsigned long locAdrs, unsigned int vmeAdrs, int size)
{
  long offset;
  int tmp_dctl=0;
...
*/
int
jlabgefDmaSendPhys(unsigned long physAdrs, unsigned int vmeAdrs, int size)
{
  int tmp_dctl=0;
  int channel=0; /* hard-coded for now */
  int nbytes=0;
  ...
  /* Make sure the destination address falls within the range of
     allocated Physical Memory */
  isValid = dmaPMemIsValid(physAdrs);
...
*/
int
jlabgefDmaSetupLL(unsigned long locAddrBase, unsigned int *vmeAddr,
...
jlabgefDmaFlush(unsigned int vmeaddr)
{
  ...
  stat = jlabgefDmaSendPhys(dmaTrashPhysBase, vmeaddr, dmaTrashSize);
  if(stat == ERROR)
    {
      return ERROR;
    }
