...
#ifdef VXWORKS
#ifdef VXWORKS_UNIV
  retVal = sysUnivDmaConfig(addrType, dataType);
#else /* tsi148 */
...
#ifdef GEFANUC
  retVal = jlabgefDmaFlush(vmeaddr);
#else
	      ...
	      unsigned int *dmaSize,unsigned int numt)
{
  int rval = 0;
#ifdef GEFANUC
  rval = jlabgefDmaSetupLL(locAddrBase,vmeAddr,dmaSize,numt);
#endif
  return rval;
}
